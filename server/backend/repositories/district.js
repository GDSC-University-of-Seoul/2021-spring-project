...
const upperDistrictStatement = `
  SELECT upper_d.district_code, upper_d.district_name, upper_d.parent_code, a.anomaly_type, COUNT(a.anomaly_type)
  ...
  INNER JOIN anomaly_log AS a
  ON c.center_id = a.center_id AND a.anomaly_type IN ('실신', '싸움', '폭행')
`;

const lowerDistrictStatement = `
  SELECT d.district_code, d.district_name, d.parent_code, a.anomaly_type, COUNT(a.anomaly_type)
  FROM district as d
  LEFT OUTER JOIN child_care_center as c
...
const pivot = (anomalies) => {
  ...
  for (let anomaly of anomalies) {
    ...
    if (!(anomaly.district_code in districts)) {
      districts[code] = {
