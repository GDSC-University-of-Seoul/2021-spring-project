...
from tracker.tracker.multitracker import (
    STrack,
    joint_stracks,
    sub_stracks,
...
class Tracker(BaseDetector):
    def __init__(self, cfg, opt=None):
        ...
        self.model_weights = cfg.get(
            "WEIGHTS", "detector/tracker/data/jde.1088x608.uncertainty.pt"
        )
    ...
    def load_model(self):
        ...
        self.model.load_state_dict(
            torch.load(self.model_weights, map_location="cpu")["model"], strict=False
        )
        ...
        if self.tracker_opt:
            if len(self.tracker_opt.gpus) > 1:
                self.model = torch.nn.DataParallel(
                    self.model, device_ids=self.tracker_opt.gpus
                ).to(self.tracker_opt.device)
            else:
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        for image_i in range(len(imgs)):
            ...
            if dets[image_i] is not None:
                det_i = scale_coords(
                    self.img_size, dets[image_i], orig_dim_list[image_i]
                )
                """Detections"""
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4], f.numpy(), 30)
                    for (tlbrs, f) in zip(det_i[:, :5], det_i[:, -self.emb_dim :])
                ]
            else:
                detections = []
            ...
            for t in self.tracked_stracks:
                ...
                if tlwh[2] * tlwh[3] > self.tracker_opt.min_box_area:
                    res = torch.tensor(
                        [image_i, tlbr[0], tlbr[1], tlbr[2], tlbr[3], ts, tid]
                    )
