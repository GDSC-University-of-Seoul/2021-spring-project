...
from efficientdet.effdet import (
    EfficientDet,
...
class EffDetDetector(BaseDetector):
    def __init__(self, cfg, opt=None):
        ...
        self.inp_dim = (
            cfg.get("INP_DIM", None)
            if cfg.get("INP_DIM", None) is not None
            else self.model_cfg.image_size
        )
    ...
    def load_model(self):
        ...
        self.model = DetBenchEval(
            net, self.model_cfg, nms_thres=self.nms_thres, max_dets=self.max_dets
        )
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        with torch.no_grad():
            ...
            scaling_factors = torch.FloatTensor(
                [
                    1.0 / min(self.inp_dim / orig_dim[0], self.inp_dim / orig_dim[1])
                    for orig_dim in orig_dim_list
                ]
            ).view(-1, 1)
            scaling_factors = (
                scaling_factors.to(args.device) if args else scaling_factors.cuda()
            )
    ...
    def detect_one_img(self, img_name):
        ...
        with torch.no_grad():
            ...
            scaling_factor = torch.FloatTensor(
                [
                    1 / min(self.inp_dim / orig_dim[0], self.inp_dim / orig_dim[1])
                    for orig_dim in img_dim_list
                ]
            ).view(-1, 1)
            scaling_factor = (
                scaling_factor.to(args.device) if args else scaling_factor.cuda()
            )
