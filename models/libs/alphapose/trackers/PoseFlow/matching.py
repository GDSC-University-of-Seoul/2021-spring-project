    ...
    for i, m_n in enumerate(matches):
        if len(m_n) != 2:
            ...
            ret = fd.write(
                "%d %d %d %d %f \n"
                % (
                    kp1[m_n[0].queryIdx].pt[0],
                    kp1[m_n[0].queryIdx].pt[1],
                    kp2[m_n[0].trainIdx].pt[0],
                    kp2[m_n[0].trainIdx].pt[1],
                    m_n[0].distance,
                )
            )
        ...
        for imgname in imgnames[:-1]:
            ...
            if not os.path.exists(cor_file) or os.stat(cor_file).st_size < 1000:
                if args.orb:
                    ...
                    # calc deep matching
                    cmd = (
                        "./deepmatching/deepmatching %s %s -nt 10 -downscale 3 -out %s/%s_%s.txt > cache"
