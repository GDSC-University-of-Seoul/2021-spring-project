...
class PoseFlowWrapper:
        ...
        for img_name in sorted(notrack.keys()):
            ...
            for bid in range(len(notrack[img_name])):
                ...
                track[img_name][bid + 1]["box_pos"] = get_box(
                    notrack[img_name][bid]["keypoints"], img_height, img_width
                )
    ...
    def step(self, img, alphapose_results):
        ...
        frame_id_list = sorted(
            [
                (int(os.path.splitext(i)[0]), os.path.splitext(i)[1])
        ...
        cor_file = os.path.join(
            self.save_match_path, "".join([prev_frame_id, "_", frame_id, "_orb.txt"])
        )
        ...
        cur_all_pids, cur_all_pids_fff = stack_all_pids(
            self.track, frame_list, len(frame_list) - 2, self.max_pid_id, self.link_len
        )
        match_indexes, match_scores = best_matching_hungarian(
            all_cors,
            cur_all_pids,
            cur_all_pids_fff,
            self.track[frame_name],
            self.weights,
            self.weights_fff,
            self.num,
            self.mag,
        ...
        for pid1, pid2 in match_indexes:
            if match_scores[pid1][pid2] > self.match_thres:
                self.track[frame_name][pid2 + 1]["new_pid"] = cur_all_pids[pid1][
                    "new_pid"
                ]
                self.max_pid_id = max(
                    self.max_pid_id, self.track[frame_name][pid2 + 1]["new_pid"]
                )
                self.track[frame_name][pid2 + 1]["match_score"] = match_scores[pid1][
                    pid2
                ]
    ...
    def final_results(self):
        ...
        for fid, frame_name in enumerate(frame_list):
            for pid in range(self.track[frame_name]["num_boxes"]):
                self.notrack[frame_name][pid]["idx"] = self.track[frame_name][pid + 1][
                    "new_pid"
                ]
        return self.notrack
