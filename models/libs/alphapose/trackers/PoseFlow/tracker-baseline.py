...
if __name__ == "__main__":
    ...
    with open(notrack_json, "r") as f:
        ...
        for imgpath in tqdm(sorted(notrack.keys())):
            ...
            for bid in range(len(notrack[imgpath])):
                ...
                track[vname][fname][bid + 1]["box_score"] = notrack[imgpath][bid][
                    "score"
                ]
                track[vname][fname][bid + 1]["box_pos"] = get_box(
                    notrack[imgpath][bid]["keypoints"], os.path.join(image_dir, imgpath)
                )
        ...
        for idx, frame_name in enumerate(frame_list[:-1]):
            ...
            for pid1, pid2 in match_indexes:
                if match_scores[pid1][pid2] > match_thres:
                    track[video_name][next_frame_name][pid2 + 1][
                        "new_pid"
                    ] = cur_all_pids[pid1]["new_pid"]
                    max_pid_id = max(
                        max_pid_id,
                        track[video_name][next_frame_name][pid2 + 1]["new_pid"],
                    )
                    track[video_name][next_frame_name][pid2 + 1][
                        "match_score"
                    ] = match_scores[pid1][pid2]
    ...
    for video_name in tqdm(track.keys()):
        ...
        for fid, frame_name in enumerate(frame_list):
            for pid in range(1, track[video_name][frame_name]["num_boxes"] + 1):
                ...
                track[video_name][frame_name][pid]["box_pose_score"] = new_score[
                    rmpe_part_ids
                ]
    ...
    for video_name in tqdm(track.keys()):
        if dataset == "val":
            ...
            if len(name) == 0:
                name = [
                    item for item in val_jsons if video_name.split("/")[-1][1:] in item
                ]
            name = name[0]
        else:
            ...
            name = [
                item
                for item in val_jsons
                if video_name.split("/")[-1].split("_")[0] == item.split("_")[0]
            ]
            ...
            if video_name.split("/")[-1].split("_")[0][:-1] == "00000":
                name = [
                    item
                    for item in val_jsons
                    if video_name.split("/")[-1].split("_")[0][1:] == item.split("_")[0]
                ]
            if len(name) == 0:
                name = [
                    item
        ...
        for fid, frame_name in enumerate(frame_list):
            ...
            final["annolist"].append(
                {
                    "image": [{"name": os.path.join(video_name, frame_name)}],
                    "annorect": [],
                }
            )
            for pid in range(1, track[video_name][frame_name]["num_boxes"] + 1):
                ...
                final["annolist"][fid]["annorect"].append(
                    {
                        "x1": [box_pos[0]],
                        "x2": [box_pos[1]],
                        "y1": [box_pos[2]],
