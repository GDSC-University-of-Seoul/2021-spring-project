...
def display_pose(imgdir, visdir, tracked, cmap):
    ...
    for imgname in tqdm(tracked.keys()):
        ...
        for pid in range(len(tracked[imgname])):
            ...
            if pose.shape[0] == 16:
                ...
                for idx in range(len(pairs)):
                    plt.plot(
                        np.clip(pose[pairs[idx], 0], 0, width),
                        np.clip(pose[pairs[idx], 1], 0, height),
                        "r-",
                        color=cmap(tracked_id),
                        linewidth=60 / alpha_ratio * np.mean(pose[pairs[idx], 2]),
        ...
        if not os.path.exists(visdir):
            os.mkdir(visdir)
        fig.savefig(
            os.path.join(visdir, imgname.split()[0] + ".png"),
            pad_inches=0.0,
            bbox_inches=extent,
            dpi=13,
        )
...
if __name__ == "__main__":
    ...
    def load_pose_boxes(img_name):
        ...
        for bid in range(len(notrack[img_name])):
            ...
            out[bid + 1]["box_pos"] = get_box(
                notrack[img_name][bid]["keypoints"], os.path.join(image_dir, img_name)
            )
            out[bid + 1]["box_pose_pos"] = np.array(
                notrack[img_name][bid]["keypoints"]
            ).reshape(-1, 3)[:, 0:2]
            out[bid + 1]["box_pose_score"] = np.array(
                notrack[img_name][bid]["keypoints"]
            ).reshape(-1, 3)[:, -1]
        return out
    ...
    with open(notrack_json, "r") as f:
        notrack = json.load(f)
        pose_boxes = parallel_process(
            [(k,) for k in sorted(notrack.keys())], load_pose_boxes, n_jobs=8
        )
    ...
    for idx, frame_name in enumerate(tqdm(frame_list[:-1])):
        ...
        for pid1, pid2 in match_indexes:
            if match_scores[pid1][pid2] > match_thres:
                track[next_frame_name][pid2 + 1]["new_pid"] = cur_all_pids[pid1][
                    "new_pid"
                ]
                max_pid_id = max(
                    max_pid_id, track[next_frame_name][pid2 + 1]["new_pid"]
                )
                track[next_frame_name][pid2 + 1]["match_score"] = match_scores[pid1][
                    pid2
                ]
