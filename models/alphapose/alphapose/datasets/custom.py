...
class CustomDataset(data.Dataset):
    ...
    def get_bgimg(self, box_h, box_w):
        ...
        if img_h <= box_h or img_w <= box_w:
            img = cv2.resize(
                img,
                (
                    int(max(img_w, (np.random.rand() * 1.8 + 1.2) * box_w)),
                    int(max(img_h, (np.random.rand() * 1.8 + 1.2) * box_h)),
                ),
            )
            img_h, img_w = img.shape[0], img.shape[1]
        ...
        if (img_h <= img_w) and (img_h - 4 > box_w) and (np.random.rand() > 0.25):
            crop_w = int(
                (img_h - 2 - max((box_w + 2), (img_h / 3))) * np.random.rand()
                + max((box_w + 2), (img_h / 3))
            )
            start_p = (img_w - crop_w) * np.random.rand()
            img = img[:, int(start_p) : int(start_p + crop_w + 1), :]
            assert img.shape[1] > box_w and img.shape[0] > img.shape[1], (
                img.shape,
                (box_w, box_h),
            )

        assert img.shape[0] > box_h and img.shape[1] > box_w, (
            img.shape,
            (box_h, box_w),
        )
    ...
    def hand_augmentation(self, img, label):
        ...
        if not self.skip_augmentation(0.3):
            ...
            h, w = img.shape[0:2]
            hand_xmin, hand_xmax, hand_ymin, hand_ymax = (
                int(round(min(handkp[:, 0]))),
                int(round(max(handkp[:, 0]))),
                int(round(min(handkp[:, 1]))),
                int(round(max(handkp[:, 1]))),
            )
            boxw_time, boxh_time = float(np.random.rand() * 2 + 4), float(
                np.random.rand() * 4 + 5
            )
            box_w, box_h = max(int((hand_xmax - hand_xmin) * boxw_time), w + 1), max(
                int((hand_ymax - hand_ymin) * boxh_time), h + 1
            )
            ...
            x, y = int(np.random.randint(0, int(bw - box_w), size=1)), int(
                np.random.randint(0, int(bh - box_h), size=1)
            )
            hd = copy.deepcopy(img)
            new_image = copy.deepcopy(background)
            ralative_x, ralative_y = int(
                np.random.randint(0, int(box_w - w), size=1)
            ), int(np.random.randint(0, int(box_h - h), size=1))
            ...
            if max_length > 1200:
                ...
                new_image = cv2.resize(
                    new_image,
                    (
                        int(round(new_image.shape[1] * scale)),
                        int(round(new_image.shape[0] * scale)),
                    ),
                )
                handkp = handkp * scale
            label["joints_3d"][:, 0:2, 0][115:136, :] = handkp
        ...
        assert label["height"] == img.shape[0] and label["width"] == img.shape[1], (
            img.shape,
